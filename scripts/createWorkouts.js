...
const validateWorkoutConfig = (config) => {
  if (!config.name || !config.description || !config.target) {
    throw new Error(`Missing required fields in workout: ${JSON.stringify(config)}`);
  }
  ...
  if (!["Male", "Female"].includes(config.gender)) {
    throw new Error(`Invalid gender value for workout: ${config.name}`);
  }
  if (!["Beginner", "Intermediate", "Advanced"].includes(config.level)) {
    throw new Error(`Invalid level value for workout: ${config.name}`);
  }
};
...
const getSetsAndReps = (level) => {
  switch (level) {
    case "Beginner":
      return { sets: 3, reps: 7 };
    case "Intermediate":
      return { sets: 4, reps: 12 };
    case "Advanced":
      return { sets: 5, reps: 14 };
    default:
    ...
    for (const config of predefinedWorkouts) {
      try {
        ...
        const exercises = await Exercise.find({ 
          target: { $in: targetArray } 
        }).limit(EXERCISE_LIMIT);

        if (exercises.length === 0) {
          console.warn(`No exercises found for target: ${targetArray}. Skipping workout: ${config.name}`);
          continue;
        }

        const { sets, reps } = getSetsAndReps(config.level);
        ...
        console.error(`Error creating workout ${config.name}:`, error.message);
      }
    }
