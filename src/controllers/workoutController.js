...
export const getAllWorkouts = async (req, res) => {
  try {
    ...
    if (isTemplate !== undefined) {
      if (isTemplate === 'true' || isTemplate === 'false' || typeof isTemplate === 'boolean') {
        query.isTemplate = String(isTemplate).toLowerCase() === 'true';
      } else {
        validationErrors.push("isTemplate must be boolean or string 'true'/'false'");
      }
    }
    ...
    if (userId) {
      if (mongoose.Types.ObjectId.isValid(userId)) {
        ...
        validationErrors.push("Invalid userId format - must be MongoDB ObjectId");
      }
    }
    ...
    if (gender) {
      if (VALID_GENDERS.includes(gender)) {
        ...
        validationErrors.push(`Invalid gender. Allowed values: ${VALID_GENDERS.join(', ')}`);
      }
    }
    ...
    if (age !== undefined) {
      ...
      if (!isNaN(ageNum) && ageNum >= MIN_AGE && ageNum <= MAX_AGE) {
        ...
        validationErrors.push(`Age must be number between ${MIN_AGE} and ${MAX_AGE}`);
      }
    }
    ...
    if (weight !== undefined) {
      ...
      if (!isNaN(weightNum) && weightNum >= MIN_WEIGHT && weightNum <= MAX_WEIGHT) {
        ...
        validationErrors.push(`Weight must be number between ${MIN_WEIGHT} and ${MAX_WEIGHT}`);
      }
    }
    ...
    const limitNumber = Math.min(MAX_LIMIT, Math.max(1, parseInt(limit, 10) || 10));
